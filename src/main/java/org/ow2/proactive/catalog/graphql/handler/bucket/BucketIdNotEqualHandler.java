...
import org.ow2.proactive.catalog.repository.specification.bucket.BucketIdNotEqualSpecification;
 ...
 * @since 12/06/2017
 */
public class BucketIdNotEqualHandler implements Handler<BucketWhereArgs, BucketEntity> {
    ...
    public Optional<Specification<BucketEntity>> handle(BucketWhereArgs bucket) {
        if (bucket.getIdArgs() != null && bucket.getIdArgs().getEq() != null) {
            return Optional.of(new BucketIdNotEqualSpecification(new Long[] { bucket.getIdArgs().getNe() }));
        }
