...
@Log4j2
@Component
public final class TextParser extends AbstractCatalogObjectParser {

    @Override
    public boolean isMyKind(String kind) {
        return kind.toLowerCase().startsWith(SupportedParserKinds.TEXT.toString().toLowerCase());
    }

    @Override
    public String getIconPath(List<KeyValueLabelMetadataEntity> keyValueMetadataEntities) {
        return SupportedParserKinds.TEXT.getDefaultIcon();
    }
