 ...
 * @since 12/06/2017
 */
public class BucketIdEqualSpecification extends IdSpecification<BucketEntity> {

    public BucketIdEqualSpecification(Long[] ids) {
        super(ids);
    }

    @Override
    public Predicate toPredicate(Root<BucketEntity> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        return cb.equal(root.get(BucketEntityMetaModelEnum.ID.getName()), ids[0]);
    }
}
